...
def entity_split(x, y, id2tag, entities, cur):
    ...
    for j in range(len(x)):
        if re.search('B',id2tag[y[j]]):
            start = cur + j
        elif re.search('I',id2tag[y[j]]) and start != -1:
            continue
        elif re.search('E',id2tag[y[j]]) and start != -1:
            end = cur + j
            entities.add((start, end))
            start, end = -1, -1
        elif re.search('O',id2tag[y[j]]) or re.search('S',id2tag[y[j]]):
...
def main(args):
    ...
    model = NER(len(word2id), tag2id, args.embedding_dim, args.hidden_dim)
    if use_cuda:
        model = model.cuda()
